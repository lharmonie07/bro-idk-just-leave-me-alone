...
def get_env_clean(key: str, default=""):
    """Получаем переменную окружения и очищаем её от пробелов, BOM и переносов."""
...
TEXT_KEYS = [
    (bytes.fromhex(get_env_clean("AES_TEXT_KEY_HEX")),
     bytes.fromhex(get_env_clean("AES_TEXT_IV_HEX")))
]
ARCHIVE_KEYS = [
    (bytes.fromhex(get_env_clean("AES_ARCHIVE_KEY_HEX")),
     bytes.fromhex(get_env_clean("AES_ARCHIVE_IV_HEX")))
]
