...
# === ЗАГРУЗКА .env (важно для локальных тестов и Scalingo) ===
load_dotenv()
...
async def cmd_start(message: types.Message):
    await message.reply(
        "Приветствую! Пришли зашифрованный файл, архив или ссылку — я его расшифрую."
    )
...
async def cmd_changes(message: types.Message):
    changes = (
        "📝 Последние изменения:\n"
        "* Добавлена поддержка загрузки по ссылке.\n"
        "* Улучшена расшифровка архивов и JSON.\n"
...
async def handle_document(message: types.Message):
    ...
    try:
        ...
        await message.reply(
            f"Ошибка при расшифровке: {e}", reply_to_message_id=message.message_id
        )
...
async def handle_link(message: types.Message):
    ...
    try:
        ...
        await message.reply(
            f"Ошибка при обработке ссылки: {e}", reply_to_message_id=message.message_id
        )
